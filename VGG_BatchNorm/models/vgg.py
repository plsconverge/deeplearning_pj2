...
class VGG_A(nn.Module):
    ...
    def __init__(self, inp_ch=3, num_classes=10, init_weights=True):
        ...
        self.features = nn.Sequential(
            # stage 1 -- [batch, 3, 32, 32] -> [batch, 64, 6, 16]
            nn.Conv2d(in_channels=inp_ch, out_channels=64, kernel_size=3, padding=1),
            nn.ReLU(True),
            nn.MaxPool2d(kernel_size=2, stride=2),

            # stage 2 -- [batch, 64, 16, 16] -> [batch, 128, 8, 8]
            nn.Conv2d(in_channels=64, out_channels=128, kernel_size=3, padding=1),
            nn.ReLU(True),
            nn.MaxPool2d(kernel_size=2, stride=2),

            # stage 3 -- [batch, 128, 8, 8] -> [batch, 256, 4, 4]
            ...
            # stage 4 -- [batch, 256, 4, 4] -> [batch, 512, 2, 2]
            ...
            # stage5 -- [batch, 512, 2, 2] -> [batch, 512, 1, 1] ** need flatten **
            nn.Conv2d(in_channels=512, out_channels=512, kernel_size=3, padding=1),
            nn.ReLU(True),
            nn.Conv2d(in_channels=512, out_channels=512, kernel_size=3, padding=1),
            nn.ReLU(True),
